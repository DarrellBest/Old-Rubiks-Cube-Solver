...
public class BreadthFirstSearch implements Runnable {
	...
	public BreadthFirstSearch(Cube cube, String threadName) {
		...
		if (threadName.contentEquals("0"))
			queue = new LinkedList<String>(Arrays.asList(" "));
	}
	...
	public void search() {
		...
		while (!queue.isEmpty()) {
			nextMove.clear();
			nextMove.addAll(Arrays.asList("F", "f", "B", "b", "L", "l", "R",
					"r", "U", "u", "D", "d"));
			// Get next moves
			moves = queue.remove();
		...
		System.out.println("Runtime in seconds: " + duration / 1000 + "."
				+ String.format("%03d", (duration % 1000)));
		// Terminates thread
		System.exit(10);
	}
	...
	private boolean isDuplicate(String moves) {
		...
		if (!retVal) {
			...
			for (int i = 0; i < 6; i++) {
				if (moves.contains(new StringBuilder().append(temp3[i])
						.append(temp1[i]).append(temp4[i]))) {
					retVal = true;
					break;
				} else if (moves.contains(new StringBuilder().append(temp4[i])
						.append(temp1[i]).append(temp3[i]))) {
					retVal = true;
					break;
				} else if (moves.contains(new StringBuilder().append(temp3[i])
						.append(temp2[i]).append(temp4[i]))) {
					retVal = true;
					break;
				} else if (moves.contains(new StringBuilder().append(temp4[i])
						.append(temp2[i]).append(temp3[i]))) {
					retVal = true;
					break;
				}
			}
